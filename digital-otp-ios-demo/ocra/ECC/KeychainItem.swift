...
public struct KeychainItem {
    ...
    func getDeviceImei() throws -> String {
        let appName = Bundle.main.infoDictionary![kCFBundleNameKey as String] as! String
        do {
            // Check for an existing uuid in the keychain.
            var query = KeychainItem.keychainQuery(withService: service, account: appName)
            query[kSecMatchLimit as String] = kSecMatchLimitOne
            query[kSecReturnAttributes as String] = kCFBooleanTrue
            query[kSecReturnData as String] = kCFBooleanTrue
            ...
            guard let existingItem = queryResult as? [String: AnyObject],
                  ...
                  let uuid = CodeableUtil.shared.decode(json: uuidData, as: String.self)
            else {
                throw KeychainError.unexpectedUserData
            }
            return uuid
        } catch KeychainError.noData {
            // Create new uuid if does not exist and save into keychain
            guard let uuid = CodeableUtil.shared.encode(object: UIDevice.current.identifierForVendor?.uuidString) else {
                fatalError("Failed to create uuid.")
            }
            
            var newItem = KeychainItem.keychainQuery(withService: service, account: appName)
            newItem[kSecValueData as String] = uuid as AnyObject?
            ...
            return CodeableUtil.shared.decode(json: uuid, as: String.self)!
        }
    }
}
